	...
	var methods = {
		...
		create: function(options) {
			...
			sub$(el).css({
				...
				sub$(this).stop().animate({'marginTop': -minus + 'px'}, 500);
			});
			var minTop = -minus + 'px';
			sub$(window).scroll(function(){
				var position = sub$(window).scrollTop();
				if(position < minTop) {
		...
		generateLinks: function(options) {
			this.links = {
				'twitter': '<li><a target="_blank" title="Twitt it!" href="http://twitter.com/home?status='+ sub$('title').html() +'+-+'+ document.location +'"><img alt="twitter" src="' + options.iconsPath + '/twitter.png"></a></li>',
				...
				'linkedin': '<li><a target="_blank" title="Share on LinkedIn" href="http://www.linkedin.com/shareArticle?mini=true&url='+ document.location +'&title='+ sub$('title').html() +'"><img alt="linkedin" src="' + options.iconsPath + '/linkedin.png"></a></li>',
